...
def estimate_text(e, name=''):
    ...
    lines = [
        f'{hello}аш предварительный расчёт',
        '━━━━━━━━━━━━━━━',
        f'🔧 {e["type_label"]}',
    ...
    lines += [
        ...
        f'⏱ Срок выполнения: ≈ {e["days"]} дн.',
        '',
        'Окончательная стоимость определяется после бесплатного замера.',
    ]
    return '\n'.join(lines)
...
# Кнопки частых вопросов. payload faq_<ключ> → ответ из FAQ_ANSWERS.
FAQ_BUTTONS = nav_buttons([
    [{'type': 'callback', 'text': '🛡 Гарантия', 'payload': 'faq_garant'},
     {'type': 'callback', 'text': '⏱ Сроки', 'payload': 'faq_srok'}],
    [{'type': 'callback', 'text': '📐 Замер', 'payload': 'faq_zamer'},
     {'type': 'callback', 'text': '💳 Оплата', 'payload': 'faq_oplata'}],
    [{'type': 'callback', 'text': '🚚 Доставка', 'payload': 'faq_dostavka'},
     {'type': 'callback', 'text': '🧱 Материалы', 'payload': 'faq_material'}],
    [{'type': 'callback', 'text': '🏠 Навесы', 'payload': 'faq_naves'},
     {'type': 'callback', 'text': '🚧 Ворота', 'payload': 'faq_vorota'}],
    [{'type': 'callback', 'text': '🎁 Акции и скидки', 'payload': 'faq_akcii'},
     {'type': 'callback', 'text': '🧾 Безнал / юрлица', 'payload': 'faq_beznal'}],
])

# Сопоставление payload кнопок FAQ → ключевое слово для match_faq().
FAQ_KEYS = {
    'garant': 'гарантия', 'srok': 'срок', 'zamer': 'замер', 'oplata': 'оплата',
    'dostavka': 'доставка', 'material': 'материал', 'naves': 'навес',
    'vorota': 'ворота', 'akcii': 'акция', 'beznal': 'безнал',
}
...
FAQ = [
    (('гарант',), '🛡 Гарантия на работы — 3 года по договору. Срок службы материалов — до 25 лет (зависит от покрытия). Выдаём гарантийный талон.'),
    (('срок', 'когда', 'быстро', 'успе'), '⏱ Срок изготовления 7–14 дней, монтаж — 1–3 дня. Замер — в день обращения.'),
    (('замер', 'выезд', 'приех'), '📐 Замер бесплатный, выезжаем в день обращения по Москве и всей МО. Замерщик посчитает точную смету на месте.'),
    (('оплат', 'рассроч', 'кредит', 'предоплат'), '💳 Оплата: 50% предоплата, 50% после монтажа и приёмки. Для юр.лиц — по безналу. Есть рассрочка.'),
    (('фундамент', 'бетон', 'грунт'), '🏗 Фундамент: присыпка щебнем (в подарок), бутование, бетонирование или ленточный — подбираем по грунту.'),
    (('достав', 'привез', 'логист'), '🚚 Доставка материалов от 6 000 ₽ по Москве и МО, своим транспортом прямо на участок. Стоимость зависит от удалённости.'),
    (('матери', 'профнаст', 'штакет', 'какой лучше', 'из чего'), '🧱 Делаем заборы из профнастила (С8, С20, С21), евроштакетника, 3D-сетки, рабицы и ковки. Профнастил — самый популярный: глухой, недорогой, долговечный. Подскажем оптимальный вариант под бюджет.'),
    (('навес', 'беседк', 'козырёк', 'козырек'), '🏠 Навесы под ключ от 3 200 ₽/м²: односкат, двускат, арочный. Покрытие — поликарбонат, профлист С21 или металлочерепица. Стандартная высота 2,2 м.'),
    (('ворот', 'откат', 'распаш', 'автоматик'), '🚧 Ворота: откатные (консольные до 8 м), распашные, секционные. Ставим автоматику DoorHan/Nice. Калитку можно добавить в комплект.'),
    (('ков', 'художеств'), '⚜️ Художественная ковка — премиум-вариант от 6 500 ₽/м.п. Эксклюзивные узоры, долговечно и эффектно. Сделаем по вашему эскизу.'),
    (('акци', 'скидк', 'дешевл', 'подарок'), '🎁 Действуют сезонные скидки до 8% и подарки (например, присыпка щебнем). Точную скидку посчитает менеджер после замера.'),
    (('безнал', 'юр.', 'юрлиц', 'ооо', 'счёт', 'счет', 'ндс'), '🧾 Работаем с юрлицами по безналу с полным пакетом документов (договор, счёт, акт). Возможна работа с НДС.'),
    (('район', 'область', 'куда', 'москв', 'удал', 'далеко'), '📍 Работаем по Москве и всей Московской области: Люберцы, Балашиха, Мытищи, Реутов и дальше. Выезд замерщика бесплатный.'),
    (('адрес', 'офис', 'находит', 'график', 'время работ'), '🕗 Принимаем заявки ежедневно. Замер и монтаж по всей МО. Оставьте номер — менеджер свяжется и согласует удобное время.'),
]
...
def build_response(conn, chat_id, text, payload, uname, settings):
    ...
    if low in ('faq', 'вопрос', 'вопросы', 'помощь', 'help'):
        out['set_stage'] = ' '
        out['reply'] = ('❓ Частые вопросы — выберите тему или просто напишите вопрос своими словами, '
                        'я постараюсь ответить:')
        out['buttons'] = FAQ_BUTTONS
        return out

    # Ответ на конкретную кнопку FAQ (faq_garant, faq_srok, ...)
    if low.startswith('faq_'):
        key = FAQ_KEYS.get(low[4:], '')
        answer = match_faq(key) if key else ''
        out['set_stage'] = ' '
        out['reply'] = answer or 'Уточните вопрос — отвечу подробнее или позову менеджера.'
        out['buttons'] = nav_buttons([
            [{'type': 'callback', 'text': '❓ Другие вопросы', 'payload': 'faq'}],
            [{'type': 'callback', 'text': '📐 Рассчитать', 'payload': 'calc_fence'},
             {'type': 'callback', 'text': '📝 Заявка', 'payload': 'order_now'}],
        ])
        return out
    ...
    if low in ('prices', 'цены', 'прайс'):
        ...
        out['buttons'] = nav_buttons([
            [{'type': 'callback', 'text': '📐 Рассчитать точно', 'payload': 'calc'}],
            [{'type': 'callback', 'text': '❓ Частые вопросы', 'payload': 'faq'}],
        ])
        return out
    ...
    if stage == 'calc_height':
        ...
        if draft.get('objectType') == 'canopy':
            ...
            out['buttons'] = nav_buttons([
                ...
                [{'type': 'callback', 'text': '❓ Вопросы по смете', 'payload': 'faq'}],
            ])
            return out
        draft['fenceHeight'] = h
        out['set_draft'] = draft
